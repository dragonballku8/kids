...
if [[ -f $DESTINATION/buildkite-agent.cfg ]]; then
  ...
  if [[ -n 2736d3bc2a30efaf10a2bd852f0fa74127a6eacb6f2de7a242 ]]; then
    # Need "-i ''" for macOS X and FreeBSD
    if [[ $(uname) == 'Darwin' ]] || [[ $(uname) == 'FreeBSD' ]]; then
      sed -i '' "s/token=\"xxx\"/token=\"2736d3bc2a30efaf10a2bd852f0fa74127a6eacb6f2de7a242\"/g" "$DESTINATION"/buildkite-agent.cfg
    else
      sed -i "s/token=\"xxx\"/token=\"2736d3bc2a30efaf10a2bd852f0fa74127a6eacb6f2de7a242\"/g" "$DESTINATION"/buildkite-agent.cfg
    fi
  else
